...
function makeEleven() {
  ...
  localStorage.setItem("arrayEleven", obj);
}
makeEleven();
//Задание 12
...
//Она должна находить span c классом practicum12, который чуть ниже задания,
// и в него с помощью метода TextContent должно записываться имя пользователя 
//из localStorage name первого пользователя из массива .
...
//Для преобразования массива в нужную строку используйте метод join().
...
//олучить сумму элементов массива и записать его в тег span c классом practicum14.
...
document.querySelector('.practicum14').textContent = count
}
addLocalStorage()
calcArr()
...
//В Лондон живёт 8900000 человек
...
renderString()
//У вас есть две функции: createUser, в которой вы должны создать обьект любого пользователя 
//и в нем обязательно должны быть поля: nameUser,ageUser, workUser, emailUser, phoneUser.
...
//Ваша задача написать условие: если этого массива нет, то создать его,
// положить туда число, что ввел пользователь и записать в localStorage под ключом task19.
//Если этот массив там есть, то нужно обратиться к нему и записать туда новое число, 
//и этот измененный массив с новым числом записать в localStorage.
...
addToNumbers();
//Создайте функцию removeLastElement, которая получает массив из localStorage
// под ключом task19 и записывает его в переменную
//После получения массива функция должна удалять последний элемент массива и возвращать 
//обновленный массив в localStorage под тем же ключом
//Важно предусмотреть условия, если в массиве больше нет элементов и он пуст, функция не должна 
